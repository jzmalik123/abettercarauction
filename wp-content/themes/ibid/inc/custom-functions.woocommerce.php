...
if (!function_exists('ibid_mobile_shop_filters')) {
	...
	function ibid_mobile_shop_filters(){
		echo '<a href="#" class="ibid-shop-filters-button btn btn-success hide-on-desktops"><i class="fas fa-filter"></i> '.esc_html__('Filters', 'ibid').'</a>';
	}
}
...
if (!function_exists('ibid_count_views')) {
	function ibid_count_views() {
	    ...
	    if ($product_pageviews) {
		    echo '<div class="mt-view-count"><i class="far fa-eye"></i>';
...
if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
		        ...
		        <?php if (isset($money_goal) && !empty($money_goal)) { ?>
		            ...
		            <div class="campaign_donation_holder col-md-12 col-xs-12">
		                <div class="campaign_summary">
	                        ...
	                        <?php echo '<div class="ibid-supported-cause"><i class="far fa-heart" aria-hidden="true"></i> '.esc_html__('Supporting: ', 'ibid').'<a class="cause-child" href="'.get_the_permalink($product_cause_id).'">'.get_the_title($product_cause_id).'</a></div>'; ?>
	...
	if (!function_exists('ibid_donation_cause_text_on_grids')) {
		function ibid_donation_cause_text_on_grids($product_id = ''){
		    ...
		    if ($ibid_redux['ibid_enable_fundraising'] == 'enable') {
		        if(!empty($product_id)){
		            echo '<div class="ibid-supported-cause"><i class="far fa-heart" aria-hidden="true"></i> '.esc_html__('Cause:','ibid').' <a class="cause-child" href="'.esc_url(get_permalink($product_id)).'" title="'. get_the_title($product_id) .'">'. get_the_title($product_id) .'</a></div>';
		        }
		    }
		}
