...
if (!function_exists('ibid_scripts')) {
    function ibid_scripts() {
        ...
        //STYLESHEETS
        wp_enqueue_style( "font-awesome5", get_template_directory_uri()."/css/plugins/font-awesome/all.min.css", array(), '5.15.4' );
                ...
                echo '<div class="overlay-components">';
                    ...
                    if (  class_exists( 'YITH_WCQV' ) ) {
                        echo '<div class="component quick-view '.esc_attr($quick_view_status).'">';
                            echo '<a href="'.esc_url('#').'" class="button yith-wcqv-button" data-tooltip="'.esc_attr__('Quickview', 'ibid').'" data-product_id="' . esc_attr(yit_get_prop( $product, 'id', true )) . '"><i class="fas fa-search"></i></a>';
                        echo '</div>';
                    }
            ...
            echo '<div class="details-price-container details-item">';
                ...
                if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
                    if (ibid_redux('ibid_enable_fundraising') != 'disable' && ibid_redux('ibid_fundraising_in_archives') != false) {
                        ...
                        if ($product_cause) {
                            echo '<div class="ibid-supported-cause"><i class="far fa-heart"></i> '.esc_html__('Supporting: ', 'ibid').'<a class="cause-child" href="'.get_the_permalink($product_cause).'">'.get_the_title($product_cause).'</a></div>';
                        }
                ...
                do_action('ibid_shop_archive_before_price');

                // Price & texts
                ibid_get_price_and_text(get_the_ID());
                
                do_action('ibid_shop_archive_after_price');
